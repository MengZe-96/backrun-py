...
class CopyTradeConfig(BaseModel):
    ...
    fast_trade_sleep_threshold: int = 5, # 快速交易次数阈值，超过该次数则暂停跟单
    fast_trade_sleep_time: int = 3600,
    # 新增统计数据
    fast_trade_time: int = 0 # fast_trade_start_time -> fast_trade_start_time+fast_trade_duration, 内快速交易的次数
    current_position: int = 0, # 当前持仓所用sol数量
