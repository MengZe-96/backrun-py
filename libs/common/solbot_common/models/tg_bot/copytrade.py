...
class CopyTrade(Base, table=True):
    ...
    # delete some fields
    priority: int = Field(nullable=False, sa_type=BIGINT, description="优先费用(单位 SOL)")
    ...
    min_buy_sol: int = Field(nullable=False, sa_type=BIGINT, description="单笔最小购买金额，以sol计价")
    max_buy_sol: int = Field(nullable=False, sa_type=BIGINT, description="单笔最大购买金额，以sol计价")
    min_sell_ratio: float = Field(nullable=False, description="单笔最小meme卖出比例")
    filter_min_buy: int = Field(nullable=False, sa_type=BIGINT, description="过滤小于该金额的买入")
    max_position: int = Field(nullable=False, sa_type=BIGINT, description="最大仓位金额，以sol计价")
    ...
    current_position: int = Field(nullable=False, sa_type=BIGINT, description="当前持仓所用sol数量")
    ...
    sol_sold: int = Field(nullable=False, sa_type=BIGINT, description="累计支出sol数量")
    sol_earned: int = Field(nullable=False, sa_type=BIGINT, description="累计收入sol数量")
